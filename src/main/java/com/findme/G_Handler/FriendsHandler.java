...
public class FriendsHandler extends Handler{
    ...
    void status(Relationship relationship, Status status) throws BadRequestException {
        if(relationship.getStatus().equals(Status.FRIENDS) && status.equals(Status.FORMER_FRIENDS)){
              if(timeSpanCheck(relationship.getDateChangeStatus()))
                      throw new BadRequestException("You can only remove from friends after 3 days from the date of friendship");
